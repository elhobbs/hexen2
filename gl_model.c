...
void Mod_LoadTextures (lump_t *l)
{
	...
	for (i=0 ; i<m->nummiptex ; i++)
	{
		...
		{
			texture_mode = GL_NEAREST;// LINEAR_MIPMAP_NEAREST; //_LINEAR;
...
// load into heap
	...
	waitforit2("Mod_LoadVertexes");
	Mod_LoadVertexes (&header->lumps[LUMP_VERTEXES]);

	waitforit2("Mod_LoadEdges");
	Mod_LoadEdges (&header->lumps[LUMP_EDGES]);

	waitforit2("Mod_LoadSurfedges");
	Mod_LoadSurfedges (&header->lumps[LUMP_SURFEDGES]);

	waitforit2("Mod_LoadTextures");
	Mod_LoadTextures(&header->lumps[LUMP_TEXTURES]);

	waitforit2("Mod_LoadLighting");
	Mod_LoadLighting (&header->lumps[LUMP_LIGHTING]);

	waitforit2("Mod_LoadPlanes");
	Mod_LoadPlanes (&header->lumps[LUMP_PLANES]);

	waitforit2("Mod_LoadTexinfo");
	Mod_LoadTexinfo (&header->lumps[LUMP_TEXINFO]);

	waitforit2("Mod_LoadFaces");
	Mod_LoadFaces (&header->lumps[LUMP_FACES]);

	waitforit2("Mod_LoadMarksurfaces");
	Mod_LoadMarksurfaces (&header->lumps[LUMP_MARKSURFACES]);

	waitforit2("Mod_LoadVisibility");
	Mod_LoadVisibility (&header->lumps[LUMP_VISIBILITY]);

	waitforit2("Mod_LoadLeafs");
	Mod_LoadLeafs (&header->lumps[LUMP_LEAFS]);

	waitforit2("Mod_LoadNodes");
	Mod_LoadNodes (&header->lumps[LUMP_NODES]);

	waitforit2("Mod_LoadClipnodes");
	Mod_LoadClipnodes (&header->lumps[LUMP_CLIPNODES]);

	waitforit2("Mod_LoadEntities");
	Mod_LoadEntities (&header->lumps[LUMP_ENTITIES]);

	waitforit2("Mod_LoadSubmodels");
	Mod_LoadSubmodels (&header->lumps[LUMP_MODELS]);


	waitforit2("Mod_MakeHull0");
	Mod_MakeHull0 ();
