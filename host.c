...
// set the time and clear the general datagram
	waitforit6("SV_ClearDatagram");
	SV_ClearDatagram ();
	
// check for new clients
	waitforit6("SV_CheckForNewClients");
	SV_CheckForNewClients ();

// read client messages
	waitforit6("SV_RunClients");
	SV_RunClients ();
	
// move things around and think
// always pause in single player if in console or menus
	if (!sv.paused && (svs.maxclients > 1 || key_dest == key_game)) {
		waitforit6("SV_Physics");
		SV_Physics();
	}

// send all messages to the clients
	waitforit6("SV_SendClientMessages");
	SV_SendClientMessages ();
}

#endif

void waitforit5(char *text) {
#if 0
	printf(text);
	printf("\npress A...");
	do {
...
// decide the simulation time
	waitforit5("Host_FilterTime");
	if (!Host_FilterTime (time))
		return;			// don't run too fast, or packets will flood out
		
// get new key events
	waitforit5("Sys_SendKeyEvents");
	Sys_SendKeyEvents ();

// allow mice or other external controllers to add commands
	waitforit5("IN_Commands");
	IN_Commands ();

// process console commands
	waitforit5("Cbuf_Execute");
	Cbuf_Execute ();
...
// check for commands typed to the host
	waitforit5("Host_GetConsoleCommands");
	Host_GetConsoleCommands ();
	waitforit5("Host_GetConsoleCommands - done");
...
// fetch results from server
	...
	{
		if (sv.active) {
			waitforit5("Host_ServerFrame 1");
			Host_ServerFrame();
			waitforit5("Host_ServerFrame 2");
		}
	...
	// fetch results from server
		if (cls.state == ca_connected)
		{
			waitforit5("CL_ReadFromServer");
			CL_ReadFromServer ();
		}


		waitforit5("R_UpdateParticles");
		R_UpdateParticles ();
		waitforit5("CL_UpdateEffects");
		CL_UpdateEffects ();
...
//		minimum_memory = MINIMUM_MEMORY_LEVELPAK;
	...
	waitforit("V_Init");
	V_Init ();
	waitforit("Chase_Init");
	Chase_Init ();
	waitforit("Host_InitVCR");
	Host_InitVCR (parms);
	waitforit("COM_Init");
	COM_Init (parms->basedir);
	waitforit("Host_InitLocal");
	Host_InitLocal ();
	waitforit("W_LoadWadFile");
