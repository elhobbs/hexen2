...
#ifdef _3DS
    glRotateZ (e->angles[1]);
    glRotateY (-e->angles[0]);
    glRotateZ (-e->angles[2]);
#else
...
//		glRotatef(-angles[2], 1, 0, 0);
	...
	else 
	{
		...
		{
			glRotateZ(anglemod((e->origin[0] + e->origin[1])*0.8 + (108 * cl.time)));
		}
		else
		{
			glRotateZ(e->angles[1]);
...
lastposenum = posenum;
	...
	while (1)
	{
		...
		do
		{
			...
			vert[i].n[2] = 1;
			vert[i].n[1] = 1;
			vert[i].xyz[0] = verts->v[0];
			vert[i].xyz[1] = verts->v[1];
			vert[i].xyz[2] = verts->v[2];
			vert[i].c[0] = r*l;
			vert[i].c[1] = g*l;
			vert[i].c[2] = b*l;
			vert[i].c[3] = model_constant_alpha;
			vert[i].st[0] = ((float *)order)[0];
			vert[i].st[1] = ((float *)order)[1];
			i++;
			order += 2;
			verts++;
		} while (--count);
...
#ifdef _3DS
	glRotateX(-90);	    // put Z going up
	glRotateZ(90);	    // put Z going up
	glRotateX(-r_refdef.viewangles[2]);
	glRotateY(-r_refdef.viewangles[0]);
	glRotateZ(-r_refdef.viewangles[1]);
