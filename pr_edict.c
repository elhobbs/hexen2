...
void PR_LoadStrings(void)
{
	...
	for(i=count=0; pr_global_strings[i] != 0; i++)
	{
		...
		{
			//printf("first: %02x\n", (int)pr_global_strings[i]);
			if (NewLineChar == pr_global_strings[i] || NewLineChar == (char)0xff)
			{
				//printf("next: %02x %02x %d\n", (int)pr_global_strings[i], (int)NewLineChar, count);
