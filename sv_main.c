...
// update frags, names, etc
	waitforit7("SV_UpdateToReliableMessages");
	SV_UpdateToReliableMessages ();
...
// build individual updates
	for (i=0, host_client = svs.clients ; i<svs.maxclients ; i++, host_client++)
	{
		...
		{
			waitforit7("SV_SendClientDatagram");
...
// clear muzzle flashes
	waitforit7("SV_CleanupEnts");
	SV_CleanupEnts ();
	waitforit7("SV_CleanupEnts - done");
}
...
// run two frames to allow everything to settle
	host_frametime = 0.1;
	waitforit2("SV_Physics 1");
	SV_Physics ();
	waitforit2("SV_Physics 2");
	SV_Physics ();

// create a baseline for more efficient communications
	waitforit2("SV_CreateBaseline");
	SV_CreateBaseline ();

// send serverinfo to all connected clients
	for (i = 0, host_client = svs.clients; i < svs.maxclients; i++, host_client++) {
		if (host_client->active) {
			waitforit2("SV_SendServerinfo");
